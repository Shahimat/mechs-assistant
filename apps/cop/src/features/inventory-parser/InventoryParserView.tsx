...
export function InventoryParserView() {
  ...
  // Синхронизируем refs в эффекте (без deps): запись ref.current во время
  // рендера — побочный эффект (react-hooks/refs). Хоткеи читают .current в
  // момент нажатия, поэтому обновления после каждого рендера достаточно.
  useEffect(() => {
  ...
  async function handleExportDiagnostic() {
    ...
    try {
      ...
      const path = await saveZip(bytes, `mechs-cop-diagnostic-${stamp}.zip`);
