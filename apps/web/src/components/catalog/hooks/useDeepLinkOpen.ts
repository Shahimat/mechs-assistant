...
export function useDeepLinkOpen<T>(
  ...
  // Синхронизируем в эффекте (без deps), а не во время рендера: запись в
  // ref.current на этапе рендера — побочный эффект, который ловит
  // react-hooks/refs. Эффект объявлен до основного ниже, поэтому свежие
  // колбэки гарантированно проставлены к моменту его запуска.
  useEffect(() => {
